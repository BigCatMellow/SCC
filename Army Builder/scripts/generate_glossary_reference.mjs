...
const keywordsReferencePath = path.join(rootDir, 'source-references', 'generated', 'part11_keywords.json');
...
function slugifyGlossaryId(name) {
  return String(name || '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
...
const keywordsReference = JSON.parse(await readFile(keywordsReferencePath, 'utf8'));

const glossaryEntries = keywordsReference
  .map((keyword) => {
    const raw = normalizeText(keyword.text || '');
    return {
      id: slugifyGlossaryId(keyword.name),
      name: keyword.name,
      type: 'Keyword',
      raw_definition: raw,
